...
if (window.location == "https://www.ingress.com/intel/?vp=f") {
	...
	if (!d) {
		...
		if (document.getElementById('header_email')) {
			...
			// it used to be regularly common to get temporary 'account not
			// enabled' messages from the intel site.
			// however, this is no longer common. more common is users getting
			// account suspended/banned - and this
			// currently shows the 'not enabled' message. so it's safer to not
			// repeatedly reload in this case
	...
	// window.PLAYER = {"ap": "123", "energy": 123, "available_invites": 123,
	// "nickname": "somenick", "team": "ENLIGHTENED||RESISTANCE"};

	window.PLAYER.email = email;
	/*
	 * alert(window.PLAYER.ap); alert(window.PLAYER.energy);
	 * alert(window.PLAYER.available_invites); alert(window.PLAYER.nickname);
	 * alert(window.PLAYER.team); alert(window.PLAYER.email);
	 */
